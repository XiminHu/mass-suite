...
class test_mssmain(unittest.TestCase):
    ...
    def test_noise_removal(self):
        ...
        assert np.where(test_scan[0].i<5000) == 0, "noise didn't properly removed"
        return
    ...
    def test_mzlocator(self):
        ...
        assert mzlist[0][0] - 117.113 <= 20 * 117.113, "selected mz is above defined error range"
    ...
    def test_peak_pick(self): # More to add during later development
        ...
        assert list(test_dict.values())[0][2] - 136350 <= 1, "intergration result error"
        return

    def test_peak_list(self): # More to add during later development
        path = '../../example_data/ex_1.mzML'
        test_scan = mssmain.get_scans(path) 
        d_test = mss.peak_list(test_scans[:200], 20)
